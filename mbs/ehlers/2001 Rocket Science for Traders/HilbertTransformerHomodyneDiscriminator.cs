...
namespace Mbst.Trading.Indicators.JohnEhlers
{
    ...
    /// </summary>
    [DataContract]
    public sealed class HilbertTransformerHomodyneDiscriminator : IHilbertTransformerCycleEstimator
    {
        ...
        [DataMember]
        private readonly int smoothingLengthPlus3HtLength;
        ...
        [DataMember]
        private double rePrevious;
        [DataMember]
        private double imPrevious;
        [DataMember]
        ...
        internal HilbertTransformerHomodyneDiscriminator(int smoothingLength = DefaultSmoothingLength, int warmUpPeriod = DefaultWarmUpPeriod,
            double alphaEmaQuadratureInPhase = DefaultAlphaEmaQuadratureInPhase, double alphaEmaPeriod = DefaultAlphaEmaPeriod)
        {
            ...
            this.warmUpPeriod = warmUpPeriod < smoothingLengthPlus3HtLength ? smoothingLengthPlus3HtLength : warmUpPeriod;
        ...
        public void Update(double value)
        {
            ...
            if (isPrimed)
            {
                ...
                double re = smoothedInPhase * smoothedInPhasePrevious + smoothedQuadrature * smoothedQuadraturePrevious;
                double im = smoothedInPhase * smoothedQuadraturePrevious - smoothedQuadrature * smoothedInPhasePrevious;
                ...
                re = EmaQuadratureInPhase(re, rePrevious);
                im = EmaQuadratureInPhase(im, imPrevious);
                rePrevious = re;
                imPrevious = im;
            ...
            else
            {
                ...
                double periodPrevious = period;
                if (smoothingLengthPlus3HtLengthMin1 == count)
                {
                    period = Constants.TwoPi / Math.Atan(im / re);//TODO: Math.Atan2(im, re)
                    if (double.IsNaN(period) || double.IsInfinity(period))
                        period = minPeriod;
