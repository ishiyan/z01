...
{
    [TestClass]
    public class CyberCycleTest
    {
        #region Test data
        /// <summary>
        /// Taken from TA-Lib Excel simulation (TALib data), test_iTrend.xsl, high price, B3…B254, 252 entries.
        /// </summary>
        ...
        /// <summary>
        /// Taken from TA-Lib Excel simulation (TALib data), test_iTrend.xsl, low price, C3…C254, 252 entries.
        /// </summary>
        ...
        /// <summary>
        /// Taken from Excel simulation, test_iTrend.xsl, (high + low)/2 median price, D3…D254, 252 entries.
        /// </summary>
        ...
        /// <summary>
        /// Taken from Excel simulation, test_iTrend.xsl, L3…L254, 252 entries.
        /// </summary>
        private readonly List<double> cycle = new List<double>
        {
        ...
        /// <summary>
        /// Taken from Excel simulation, test_iTrend.xsl, N3…N254, 252 entries.
        /// </summary>
        ...
        {
            var target = new CyberCycle();
            Assert.AreEqual("cc", target.Name);
        }
        #endregion
        ...
        {
            var target = new CyberCycle();
            Assert.AreEqual("cc(28)", target.Moniker);
            target = new CyberCycle(3, 2);
            Assert.AreEqual("cc(3)", target.Moniker);
            target = new CyberCycle(2, 11);
            Assert.AreEqual("cc(2)", target.Moniker);
        }
        #endregion
        ...
        {
            var target = new CyberCycle();
            Assert.AreEqual("Cyber Cycle", target.Description);
        }
        #endregion
        ...
        public void LengthTest()
        {
            ...
            target = new CyberCycle(3, 11);
            Assert.AreEqual(3, target.Length);
            target = new CyberCycle(2, 12);
            Assert.AreEqual(2, target.Length);
        }
        #endregion

        #region SignalLagTest
        /// <summary>
        /// A test for SignalLag.
        /// </summary>
        [TestMethod]
        public void SignalLagTest()
        {
            var target = new CyberCycle();
            Assert.AreEqual(9, target.SignalLag);
            target = new CyberCycle(3, 11);
            Assert.AreEqual(11, target.SignalLag);
            target = new CyberCycle(2, 33);
            Assert.AreEqual(33, target.SignalLag);
        }
        #endregion
        ...
        public void ValueFacadeTest()
        {
            ...
            Assert.AreEqual("cc", facade.Name);
            Assert.AreEqual("cc(28)", facade.Moniker);
            Assert.AreEqual("Cyber Cycle", facade.Description);
            for (int i = 0; i < 7; ++i)
            {
            ...
            for (int i = 7; i < rawInput.Count; ++i)
            {
                target.Update(rawInput[i]);
        ...
        #region SignalFacadeTest
        /// <summary>
        /// A test for SignalFacade.
        /// </summary>
        [TestMethod]
        public void SignalFacadeTest()
        {
            var target = new CyberCycle();
            var facade = target.SignalFacade;
            Assert.AreEqual("ccSignal", facade.Name);
            Assert.AreEqual("ccSignal(9)", facade.Moniker);
            Assert.AreEqual("Cyber Cycle signal", facade.Description);
            for (int i = 0; i < 7; ++i)
            {
                target.Update(rawInput[i]);
                Assert.IsTrue(double.IsNaN(facade.Update(rawInput[i])));
            ...
            {
                target.Update(rawInput[i]);
                Assert.AreEqual(target.Signal, facade.Update(rawInput[i]));
                Assert.IsTrue(facade.IsPrimed);
            }
        }
        #endregion
        ...
        public void SignalTest()
        {
            ...
            for (int i = 0; i < rawInput.Count; ++i)
            {
                ...
                {
                    Assert.IsTrue(double.IsNaN(target.Signal));
                    Assert.IsFalse(target.IsPrimed);
                    continue;
                }
                double d = Math.Round(target.Signal, digits);
                double u = Math.Round(signal[i], digits);
                Assert.AreEqual(u, d);
                Assert.IsTrue(target.IsPrimed);
            }
        }
        ...
        public void ValueTest()
        {
            ...
            for (int i = 0; i < rawInput.Count; ++i)
            {
                ...
                double u = Math.Round(cycle[i], digits);
                Assert.AreEqual(u, d);
                Assert.IsTrue(target.IsPrimed);
            }
        }
        ...
        public void UpdateTest()
        {
            ...
            for (int i = 0; i < 50; ++i)
            {
                ...
                double u = Math.Round(cycle[i], digits);
                Assert.AreEqual(u, d);
                d = Math.Round(target.Signal, digits);
                u = Math.Round(signal[i], digits);
                Assert.AreEqual(u, d);
            }
        ...
        {
            var target = new CyberCycle(5, 29);
            Assert.AreEqual("[M:cc(5) P:False V:NaN S:NaN]", target.ToString());
            target = new CyberCycle(13, 74);
            Assert.AreEqual("[M:cc(13) P:False V:NaN S:NaN]", target.ToString());
        }
        #endregion
        ...
        /// </summary>
        [TestMethod]
        public void CyberCycleConstructorTest()
        {
            var target = new CyberCycle();
            Assert.AreEqual(9, target.SignalLag);
            Assert.AreEqual(28, target.Length);
            ...
            target = new CyberCycle(0.06, 11, OhlcvComponent.TypicalPrice);
            Assert.AreEqual(11, target.SignalLag);
            ...
            target = new CyberCycle(28, 14);
            Assert.AreEqual(14, target.SignalLag);
            Assert.AreEqual(28, target.Length);
            Assert.IsTrue(double.IsNaN(target.Value));
            Assert.IsTrue(double.IsNaN(target.Signal));
            Assert.IsFalse(target.IsPrimed);
            ...
            target = new CyberCycle(28, 15, OhlcvComponent.OpeningPrice);
            Assert.AreEqual(15, target.SignalLag);
            Assert.AreEqual(28, target.Length);
            Assert.IsTrue(double.IsNaN(target.Value));
            Assert.IsTrue(double.IsNaN(target.Signal));
            Assert.IsFalse(target.IsPrimed);
            ...
            target = new CyberCycle(1, 1);
            Assert.AreEqual(1, target.SignalLag);
            ...
            target = new CyberCycle(2, 2, OhlcvComponent.OpeningPrice);
            Assert.AreEqual(2, target.SignalLag);
            Assert.AreEqual(2, target.Length);
            Assert.IsTrue(double.IsNaN(target.Value));
            Assert.IsTrue(double.IsNaN(target.Signal));
            Assert.IsFalse(target.IsPrimed);
        ...
        public void CyberCycleConstructorTest2()
        {
            var target = new CyberCycle(0, 1);
            Assert.IsFalse(target.IsPrimed);
        }
        ...
        [TestMethod]
        [ExpectedException(typeof(ArgumentOutOfRangeException))]
        public void CyberCycleConstructorTest3()
        {
            var target = new CyberCycle(1, 0);
            Assert.IsFalse(target.IsPrimed);
        }
        ...
        [TestMethod]
        [ExpectedException(typeof(ArgumentOutOfRangeException))]
        public void CyberCycleConstructorTest4()
        {
            var target = new CyberCycle(0, 0);
            Assert.IsFalse(target.IsPrimed);
        }
        ...
        [TestMethod]
        [ExpectedException(typeof(ArgumentOutOfRangeException))]
        public void CyberCycleConstructorTest5()
        {
            var target = new CyberCycle(-8, 1);
            Assert.IsFalse(target.IsPrimed);
        }
        ...
        [TestMethod]
        [ExpectedException(typeof(ArgumentOutOfRangeException))]
        public void CyberCycleConstructorTest6()
        {
            var target = new CyberCycle(1, -8);
            Assert.IsFalse(target.IsPrimed);
        }
        ...
        [TestMethod]
        [ExpectedException(typeof(ArgumentOutOfRangeException))]
        public void CyberCycleConstructorTest7()
        {
            var target = new CyberCycle(-8, -9);
            Assert.IsFalse(target.IsPrimed);
        }
        ...
        [TestMethod]
        [ExpectedException(typeof(ArgumentOutOfRangeException))]
        public void CyberCycleConstructorTest8()
        {
            var target = new CyberCycle(0.07, 0);
            Assert.IsFalse(target.IsPrimed);
        }
        ...
        [TestMethod]
        [ExpectedException(typeof(ArgumentOutOfRangeException))]
        public void CyberCycleConstructorTest9()
        {
            var target = new CyberCycle(-0.0001, 8);
            Assert.IsFalse(target.IsPrimed);
        }
        ...
        [TestMethod]
        [ExpectedException(typeof(ArgumentOutOfRangeException))]
        public void CyberCycleConstructorTest10()
        {
            var target = new CyberCycle(1.0001, 8);
            Assert.IsFalse(target.IsPrimed);
        }
        ...
        [TestMethod]
        [ExpectedException(typeof(ArgumentOutOfRangeException))]
        public void CyberCycleConstructorTest11()
        {
            var target = new CyberCycle(0.07, -8);
            Assert.IsFalse(target.IsPrimed);
        }
        #endregion

        #region SerializationTest
        private static void SerializeTo(CyberCycle instance, string fileName)
        {
            var dcs = new DataContractSerializer(typeof(CyberCycle), null, 65536, false, true, null);
        ...
        private static CyberCycle DeserializeFrom(string fileName)
        {
            var fs = new FileStream(fileName, FileMode.Open);
            XmlDictionaryReader reader = XmlDictionaryReader.CreateTextReader(fs, new XmlDictionaryReaderQuotas());
            var ser = new DataContractSerializer(typeof(CyberCycle), null, 65536, false, true, null);
            var instance = (CyberCycle)ser.ReadObject(reader, true);
            reader.Close();
            fs.Close();
            return instance;
        }
        ...
        public void SerializationTest()
        {
            ...
            for (int i = 0; i < 111; ++i)
            {
                ...
                u = Math.Round(cycle[i], digits);
                Assert.AreEqual(u, d);
                d = Math.Round(source.Signal, digits);
                u = Math.Round(signal[i], digits);
                Assert.AreEqual(u, d);
            }
            const string fileName = "CyberCycle_1.xml";
            SerializeTo(source, fileName);
            CyberCycle target = DeserializeFrom(fileName);
            Assert.AreEqual(source.Value, target.Value);
            Assert.AreEqual(source.Signal, target.Signal);
