...
{
    /// <summary>
    /// Implements an autoregressive (AR) model to estimate the AR coefficients using Burg maximum entropy method.
    /// <para/>
    /// <para/>
    /// </summary>
    [DataContract]
    internal sealed class MaximumEntropySpectrumEstimator
    {
        ...
        /// <summary>
        /// The degree (order) of autoregression.
        /// </summary>
        [DataMember]
        ...
        public void Calculate()
        {
            ...
            for (int i = 0; i < LengthSpectrum; ++i)
            {
                // Calculates a spectrum value at a given frequency (in radians) from an autoregression vector.
                double real = 1.0;
                double imag = 0.0;
                for (int j = 0; j < Degree; ++j)
                {
